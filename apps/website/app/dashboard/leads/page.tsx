...
const WORKSPACE_ID = "1861ee99-3966-42c9-ac31-b9cb092d7f67";
...
const leads = await getLeadResponseQueue(WORKSPACE_ID);

const newLeads = leads.filter((lead) => lead.status === "new").length;
const contacted = leads.filter((lead) => lead.firstContactedAt).length;

return ( <main className="min-h-screen bg-slate-50 px-5 py-8 text-slate-950 sm:px-8"> <div className="mx-auto max-w-7xl"> <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"> <div> <Link
           href="/dashboard"
...
← Dashboard </Link>
        ...
        <p className="mt-2 max-w-2xl text-slate-600">
          The two-minute clock begins when a visitor submits their
          details. Mark contact only after a real human call, email, or
          WhatsApp response.
        </p>
      </div>
    ...
    <div className="mt-8 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
      {leads.length === 0 ? (
        <div className="p-10 text-center text-slate-600">
          No captured leads yet. New PromptProfit captures will appear here
          automatically.
        </div>
      ) : (
